    ...
    <div className="flex flex-col h-screen bg-gray-100">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {loading ? <p className="text-center mt-10">Loading data...</p> : !selectedWO ? (
          <div>
            <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Tugas Hari Ini</h2>
            {workOrders.length === 0 ? <p className="text-gray-500 text-center mt-10">Tidak ada tugas.</p> : (
              <div className="space-y-3">
                {workOrders.map(wo => (
                  <div key={wo.id} className="bg-white border p-4 rounded-xl shadow-sm" onClick={() => setSelectedWO(wo)}>
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-bold text-gray-800">{wo.customer_name || 'Pelanggan Baru'}</span>
                      <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full text-white ${wo.status === 'completed' ? 'bg-green-500' : wo.status === 'working' ? 'bg-orange-500' : 'bg-gray-400'}`}>
                        {wo.status.toUpperCase()}
                      </span>
                    </div>
                    <p className="text-xs text-gray-600 mb-1">{wo.customer_address}</p>
                    <p className="text-xs font-medium text-blue-600">{wo.type === 'pasang_baru' ? 'Pasang Baru' : 'Gangguan'}</p>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm flex flex-col gap-4">
            ...
            <div className="text-sm space-y-1">
              ...
              <p className="mt-2 text-gray-600">{selectedWO.description}</p>
            </div>
