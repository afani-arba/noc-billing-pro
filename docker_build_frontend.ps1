...
SSH "docker run --rm -v /opt/noc-billing-pro:/repo -w /repo alpine sh -c 'git config --global --add safe.directory /repo && git fetch origin main && git reset --hard origin/main && echo PULLED_OK' 2>&1"
...
SSH "docker run --rm -v /opt/noc-billing-pro/frontend:/app -w /app node:20-alpine sh -c 'npm ci --silent && npm run build 2>&1 | tail -10 && echo BUILD_OK' 2>&1"
