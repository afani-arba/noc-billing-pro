#!/bin/bash
dos2unix /home/noc/install.sh 2>/dev/null
chmod +x /home/noc/install.sh
mkdir -p /opt/noc-billing-pro/backend
cat > /opt/noc-billing-pro/backend/.env << 'ENVEOF'
MONGO_URI=mongodb://mongodb:27017/nocbillingpro
MONGO_DB_NAME=nocbillingpro
SECRET_KEY=b2c5e523f66c06a3
ACCESS_TOKEN_EXPIRE_MINUTES=1440
NOC_SERVICE_NAME=NOC Billing Pro
APP_URL=http://$(hostname -I | awk '{print $1}'):8082
APP_EDITION=billing_pro
CORS_ORIGINS=*
ENABLE_BILLING_SCHEDULER=true
ENABLE_ISOLIR=true
ENABLE_HOTSPOT_CLEANUP=true
ENABLE_POLLING=true
ENABLE_SSE=true
ENABLE_SYSLOG=true
SYSLOG_PORT=5142
ENABLE_BACKUP=true
ENABLE_ROUTING_ALERTS=true
ENABLE_SPEEDTEST=true
ENABLE_SESSION_CACHE=true
ENABLE_SNMP_POLLER=true
ENABLE_RADIUS=true
RADIUS_SECRET=ArbaTraining
ENABLE_GENIEACS_SYNC=true
GENIEACS_URL=http://genieacs-nbi:7557
GENIEACS_USERNAME=admin
GENIEACS_PASSWORD=admin
ENABLE_BGP_STEERING=true
GOBGPD_HOST=172.18.0.1
ENABLE_ROUTE_OPTIMIZER=false
ENABLE_NETWATCH_POLLER=false
ENABLE_NETFLOW=false
LICENSE_SERVER_URL=https://license.arbatraining.com
CF_TUNNEL_TOKEN=
FIREBASE_CREDENTIALS_PATH=/app/firebase-service-account.json
ENVEOF

bash /home/noc/install.sh
